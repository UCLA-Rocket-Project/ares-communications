...
fsmptlabels = ["FuelManifold", "OxManifold", "Combustion Chamber", "FSM: PT4", "FSM: PT5", "FSM: PT6", "FSM: PT7", "FSM: PT8", "FSM: PT9"]
#gsmptlabels = ["GSM: PT1" , "GSM: PT2", "GSM: PT3", "GSM: PT4", "GSM: PT5", "GSM: PT6", "GSM: PT7", "GSM: PT8", "GSM: PT9"]
gsmptlabels = ["PresTank" , "OxTank", "FuelTank", "GSM: PT4", "GSM: PT5", "GSM: PT6", "GSM: PT7", "GSM: PT8", "GSM: PT9"]
#fsmtclabels = ["FSM: TC1", "FSM: TC2", "FSM: TC3", "FSM: TC4", "FSM: TC5", "FSM: TC6", "FSM: TC7", "FSM: TC8", "FSM: TC9"]
fsmtclabels = ["FSM: TC1", "Ambient", "FSM: TC3", "FSM: TC4", "FSM: TC5", "FSM: TC6", "FSM: TC7", "FSM: TC8", "FSM: TC9"]
#gsmtclabels = ["GSM: TC1", "GSM: TC2", "GSM: TC3", "GSM: TC4", "GSM: TC5", "GSM: TC6", "GSM: TC7", "GSM: TC8", "GSM: TC9"]
gsmtclabels = ["GSM: TC1", "CCCenter", "Lower1/4Ox", "OxVent", "Middle1/4Ox", "Top1/4Ox", "GSM: TC7", "GSM: TC8", "GSM: TC9"]

#has to have 3 values
fsmptoffset = [1, 16.379, 17.43]
fsmptscale = [1, 1.0557, 0.5287]
gsmptoffset = [-29.808, 16.464, 17.148]
gsmptscale = [0.998, 0.5263, 1.0488]
...
def animate(i, FSMq, GSMq, EXITq):
    global refreshRate
    global force_update_counter
    global exitall

    if exitall == True:
        print ("Shutting visualization down!")
        EXITq.put(0)
        exit()
    ...
    try:
        if graph == "Pressure Transducer":
            ...
            if drawgraph:
                ...
                gridgraph.clear()
                for i in range (3, len(gsmpressurelist)):
                    gridgraph.plot(np.array([float(j) for j in gsmpressurelist[0]]), np.array([((float(j) * gsmptscale[i - 2]) + gsmptoffset[i - 2]) for j in gsmpressurelist[i]]), gsmptcolors[i-2], label = gsmptlabels[i-2])
                for i in range (2, len(fsmpressurelist)):
                    gridgraph.plot(np.array([float(j) for j in fsmpressurelist[0]]), np.array([((float(j) * fsmptscale[i - 2]) + fsmptoffset[i - 2]) for j in fsmpressurelist[i]]), fsmptcolors[i-2], label = fsmptlabels[i-2])
                

                for i in range (2, len(gsmthermolist)):
                	gridgraph.text(-0.16, 1.05 - 0.05 * i, gsmtclabels[i-2] + ": " + gsmthermolist[i][len(gsmthermolist[i]) - 1], size = 8, verticalalignment = 'center', horizontalalignment = 'left', transform = gridgraph.transAxes)
                for i in range (2, len(fsmthermolist)):
                	gridgraph.text(-0.16, 0.7 - 0.05 * i, fsmtclabels[i-2] + ": " + fsmthermolist[i][len(fsmthermolist[i]) - 1], size = 8, verticalalignment = 'center', horizontalalignment = 'left', transform = gridgraph.transAxes)
                for i in range (2, len(gsmpressurelist)):
                    gridgraph.text(-0.16, 0.05 - 0.05 * i, gsmptlabels[i-2] + ": " + str((float(gsmpressurelist[i][len(gsmpressurelist[i]) - 1]) * gsmptscale[i - 2]) + gsmptoffset[i - 2]), size = 8, verticalalignment = 'center', horizontalalignment = 'left', transform = gridgraph.transAxes)
                for i in range (2, len(fsmpressurelist)):
                    gridgraph.text(-0.16, -0.10 - 0.05 * i, fsmptlabels[i-2] + ": " + str((float(fsmpressurelist[i][len(fsmpressurelist[i]) - 1]) * fsmptscale[i - 2])  + fsmptoffset[i - 2]), size = 8, verticalalignment = 'center', horizontalalignment = 'left', transform = gridgraph.transAxes)
                ...
                gridgraph2 = gridgraph.twinx()
                gridgraph2.plot(np.array([float(j) for j in gsmpressurelist[0]]), np.array([((float(j) * gsmptscale[0]) + gsmptoffset[0]) for j in gsmpressurelist[2]]), gsmptcolors[0], label = gsmptlabels[0])
                #gridgraph2.set_ylim([-200, 11000])
                gridgraph2.xaxis.set_major_locator(mticker.MaxNLocator(num_markers))
                gridgraph2.legend(bbox_to_anchor=(0.5, -0.375, 0, 0), loc=3, ncol=3, borderaxespad=0)
        ...
        if graph == "Thermocouple":
            ...
            if drawgraph:
                ...
                for i in range (3, len(fsmthermolist)):
                ...
                for i in range (2, len(fsmthermolist)):
                    gridgraph.text(-0.16, 0.7 - 0.05 * i, fsmtclabels[i-2] + ": " + fsmthermolist[i][len(fsmthermolist[i]) - 1], size = 8, verticalalignment = 'center', horizontalalignment = 'left', transform = gridgraph.transAxes)
                for i in range (2, len(gsmpressurelist)):
                    gridgraph.text(-0.16, 0.05 - 0.05 * i, gsmptlabels[i-2] + ": " + gsmpressurelist[i][len(gsmpressurelist[i]) - 1], size = 8, verticalalignment = 'center', horizontalalignment = 'left', transform = gridgraph.transAxes)
                for i in range (2, len(fsmpressurelist)):
                    gridgraph.text(-0.16, -0.10 - 0.05 * i, fsmptlabels[i-2] + ": " + fsmpressurelist[i][len(fsmpressurelist[i]) - 1], size = 8, verticalalignment = 'center', horizontalalignment = 'left', transform = gridgraph.transAxes)
...
class CtrlBox(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        file_menu.add_command(label="Exit", command=lambda: killall())
...
def drawgraph(FSMq, GSMq, EXITq):
    root = CtrlBox()
    root.geometry(application_size)
    ani = animation.FuncAnimation(figureFrame, animate, fargs = (FSMq, GSMq, EXITq), interval=50)
    root.mainloop()

def TCPClient(FSMq, GSMq, EXITq):
    ...
    while True:
        if not EXITq.empty():
            print ("Shutting TCP communications down!")
            EXITq.put(0)
            exit()
...
if __name__ == '__main__':
    ...
    GSMq = Queue()
    EXITq = Queue()
    p1 = Process(target = TCPClient, args = (FSMq, GSMq, EXITq,))
    p2 = Process(target = drawgraph, args = (FSMq, GSMq, EXITq,))
    p1.start()
    p2.start()


    while not EXITq.qsize() > 1:
        pass
    p1.terminate()
    p2.terminate()
